       ...
       // Duplicate the ARCoachingGoal enum so that we can use it on a serialized field
        ...
        void OnEnable()
        {
            ...
            if (supported && GetComponent<ARSession>().subsystem is ARKitSessionSubsystem sessionSubsystem)
            {
                ...
                //For Apple oriented devices, which support Coaching, activate the Coaching overlay to start.
                //Once it completes (a target tracking plane is found, etc.) the Session Delegate will handle
                //telling the GameManager that Coaching is completed, and the app controls can be enabled.
    ...
    #endif
            {
                ...
                //Essentially for non Apple devices, where Coaching is not a factor, just indicate that
                //Coaching is completed so the app controls can start up (ARCore/Android, etc.)
